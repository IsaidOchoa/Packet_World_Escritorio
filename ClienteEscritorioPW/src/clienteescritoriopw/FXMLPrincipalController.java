...
public class FXMLPrincipalController implements Initializable {
    ...
    @FXML private Button btColaboradores;
    @FXML private Button btSucursales;
    @FXML private Button btClientes;
    @FXML private Button btEnvios;
    @FXML private Button btUnidades;
    
    @FXML private ImageView ivColaborador;
    ...
    public void cargarInformacion(Colaborador colaborador) {
        ...
        if (colaborador.getFotoBase64() != null && !colaborador.getFotoBase64().isEmpty()) {
            try {
                ...
                String base64Limpio = colaborador.getFotoBase64()
                    .replaceAll("\\s", "")
                    .replaceAll("[^A-Za-z0-9+/=]", "");

                if (!base64Limpio.isEmpty()) {
                    byte[] imagenBytes = Base64.getDecoder().decode(base64Limpio);
                    Image imagen = new Image(new ByteArrayInputStream(imagenBytes));
                    ivColaborador.setImage(imagen);
                } else {
                    throw new IllegalArgumentException("Base64 vacío después de limpiar");
                }
            } catch (Exception e) {
                System.err.println("Error al cargar la foto: " + e.getMessage());
                ivColaborador.setImage(new Image(getClass().getResourceAsStream("/clienteescritoriopw/recursos/avatar.png")));
            }
        } else {
            ivColaborador.setImage(new Image(getClass().getResourceAsStream("/clienteescritoriopw/recursos/avatar.png")));
        }
